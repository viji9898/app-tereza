...
const imageAssets = {
  ...
  bathroom: "https://customer-apps-techhq.s3.eu-west-2.amazonaws.com/app-kurulu-bay-retreat/girl-opening-window-dark-room.jpg",
  handunugodaTeaEstate: "https://customer-apps-techhq.s3.eu-west-2.amazonaws.com/app-kurulu-bay-retreat/handunugoda-tea-estate.webp",
  koggalaBoatTrip: "https://customer-apps-techhq.s3.eu-west-2.amazonaws.com/app-kurulu-bay-retreat/kogola-boat-trip.jpg",
  yatalagaTemple: "https://customer-apps-techhq.s3.eu-west-2.amazonaws.com/app-kurulu-bay-retreat/yatagala-temple.jpg",
  lagoonPool: "https://img.fitreisen.?signature=7dbb579a087e444670fd4e1b525a01c7cbc0a10f24a90296963649186721ff0a",
};
...
const SectionLabel = ({ children, index }) => (
  <p className="kurulu-bay-page__eyebrow">{index ? `${index} ` : ""}{children}</p>
);

const FullImage = ({ src, alt, priority, overlayIndex, overlayLabel, overlayTitle, overlayText }) => (
  <div className={`kurulu-bay-page__media${priority ? " kurulu-bay-page__media--hero" : ""}${overlayText ? " kurulu-bay-page__media--with-overlay" : ""}`}>
    <img src={src} alt={alt} loading={priority ? "eager" : "lazy"} />
    {overlayText ? (
      <div className="kurulu-bay-page__media-overlay">
        {overlayLabel ? <SectionLabel index={overlayIndex}>{overlayLabel}</SectionLabel> : null}
        {overlayTitle ? <h2 className="kurulu-bay-page__media-overlay-title">{overlayTitle}</h2> : null}
        {Array.isArray(overlayText) ? (
          <div className="kurulu-bay-page__media-overlay-text-group">
            {overlayText.map((line) => (
              <p key={line} className="kurulu-bay-page__media-overlay-text">{line}</p>
            ))}
          </div>
        ) : (
...
export default function KuruluBayRetreat() {
  ...
  const menuItems = useMemo(() => sectionMenu.map(([id, label]) => ({ id, label })), []);
    ...
    <div className="kurulu-bay-page">
      <section id="cover" className="kurulu-bay-page__hero">
        <div className="kurulu-bay-page__hero-image">
          <img src={imageAssets.cover} alt="Kurulu Bay pool framed by tropical palms" loading="eager" />
        </div>
        <div className="kurulu-bay-page__hero-copy">
          ...
          <p className="kurulu-bay-page__meta">A 10-Day Private Retreat curated by Tereza Dos Santos</p>
          <p className="kurulu-bay-page__meta kurulu-bay-page__meta--spaced">September 21 — September 30, 2026</p>
      ...
      >
        ...
        <nav className="kurulu-bay-page__contents-nav" aria-label="Kurulu Bay retreat sections">
          {menuItems.map((item) => (
            <a
              key={item.id}
              href={`#${item.id}`}
      ...
      <section id="welcome" className="kurulu-bay-page__text-section">
        ...
        <div className="kurulu-bay-page__columns">
          ...
          <div>
            ...
            <p>My greatest hope is not that you leave feeling inspired for a few days, but that you return home carrying one small practice, one new perspective, or one feeling that truly becomes a part of the way you live. Not something you simply understand with your mind, but something you've truly embodied.</p>
            <p>If even one small part of this journey stays with you long after we've said goodbye, then I believe we've created something truly meaningful together.</p>
      ...
      <section id="return" className="kurulu-bay-page__text-section">
        ...
        <p>Over ten carefully curated days, we'll move through practices rooted in nervous system regulation, somatic experiencing, movement, sound, nature and human connection.</p>
        <p>Not everything will be revealed in advance. Some experiences are meant to be discovered only when the moment arrives.</p>
        <p>Surrender to the Mystery. Each day explores a different theme, thoughtfully designed to build upon the one before it - creating a journey that comes full circle before you return home.</p>
      </section>
      ...
      <section id="typical-day" className="kurulu-bay-page__text-section kurulu-bay-page__text-section--compact">
      ...
      <section id="in-between" className="kurulu-bay-page__text-section">
        <div className="kurulu-bay-page__lede-group">
          <p>Some of the most meaningful parts of the retreat happen between the sessions.</p>
        </div>
        ...
        <div className="kurulu-bay-page__immersion-grid">
          {immersionCards.map((item) => (
            <figure key={item.title} className="kurulu-bay-page__immersion-card">
              ...
              {item.overlayTitle ? (
                <div className={`kurulu-bay-page__immersion-card-overlay${item.overlayClassName ? ` ${item.overlayClassName}` : ""}`}>
      ...
      <section id="investment" className="kurulu-bay-page__text-section kurulu-bay-page__text-section--compact">
        <SectionLabel index="12">INVESTMENT</SectionLabel>
        <h2 className="kurulu-bay-page__headline kurulu-bay-page__headline--medium">Choose your home</h2>
        <p className="kurulu-bay-page__lead">A limited number of beautifully designed rooms are available. Each has been thoughtfully selected to provide comfort, privacy, and a peaceful place to rest and integrate throughout the journey.</p>
        <div className="kurulu-bay-page__investment-grid">
          <figure className="kurulu-bay-page__investment-image">
            <img src={imageAssets.treehouse} alt="The Treehouses at Kurulu Bay" loading="lazy" />
            <figcaption>The Treehouses</figcaption>
          </figure>
          <figure className="kurulu-bay-page__investment-image">
            <img src={imageAssets.bedroom} alt="The Garden Suites at Kurulu Bay" loading="lazy" />
            <figcaption>The Garden Suites</figcaption>
          </figure>
          <figure className="kurulu-bay-page__investment-image">
            <img src={imageAssets.cottages} alt="The Cottages at Kurulu Bay" loading="lazy" />
      ...
      <section id="closing" className="kurulu-bay-page__closing">
        <div>
          <p className="kurulu-bay-page__brand kurulu-bay-page__brand--small">THE RETURN</p>
          <div className="kurulu-bay-page__rule" />
